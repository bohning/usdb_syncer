...
def get_usdb_page(
    ...
    if method == 'GET':
        ...
        response = requests.post(
            url, headers=_headers, data=payload, params=params
        )
    else:
...
def get_usdb_available_songs(
    content_filter: Dict[str, str] = None
) -> List[dict]:
    ...
    for match in matches:
        song_id, artist, title, edition, goldennotes, language, rating_string, views = match
        song = {
            'id': song_id,
            'artist': artist,
            'title': title,
...
def _parse_song_details(details: Dict[str, str],
                        details_table: BeautifulSoup) -> Dict[str, str]:
    ...
    details['song_check'] = str(
        'Yes' in details_table.find(text='Songcheck').next.text
    )
