...
class Job(Enum):
    ...
    AUDIO_DOWNLOAD = member(partial(_maybe_download_audio))
    VIDEO_DOWNLOAD = member(partial(_maybe_download_video))
    COVER_DOWNLOAD = member(partial(_maybe_download_cover))
    BACKGROUND_DOWNLOAD = member(partial(_maybe_download_background))
    # write txt after all file downloads to include correct filenames
    TXT_WRITTEN = member(partial(_maybe_write_txt))
    WRITE_AUDIO_TAGS = member(partial(_maybe_write_audio_tags))
    WRITE_VIDEO_TAGS = member(partial(_maybe_write_video_tags))
